...
import 'api_client.dart'; // Menghubungkan ke ApiClient pengganti DbHelper

void main() {
  // Init binding SQLite dihapus karena tidak diperlukan untuk HTTP Package
  runApp(const MyApp());
}
...
// =========================================================================
// MODEL DATA (Mendukung Serialisasi JSON ↔ Objek Dart)
// =========================================================================
class Catatan {
  final int? id;
  final String judul;
  ...
  // Mengubah objek Dart menjadi JSON Map untuk dikirim ke server Laravel via HTTP
  Map<String, dynamic> toJson() => {
    if (id != null) 'id': id,
    'judul': judul,
    'isi': isi,
    'kategori': kategori,
    // Catatan: Jika server backend tidak menyediakan kolom email, data tetap terkirim dengan aman
    'dibuat_pada': dibuatPada.toUtc().toIso8601String(), // Menggunakan format ISO-8601 String
  };

  // Mengubah data JSON Map dari REST API server kembali menjadi Objek Dart
  static Catatan fromJson(Map<String, dynamic> m) => Catatan(
    id: m['id'] as int?,
    judul: m['judul'] as String,
    isi: m['isi'] as String,
    kategori: m['kategori'] as String,
    email: (m['email'] as String?) ?? 'mahasiswa@unpas.ac.id', // Fallback default jika tidak ada di DB server
    dibuatPada: DateTime.parse(m['dibuat_pada'] as String).toLocal(), // Parsing dari ISO-8601 string
  );
...
// =========================================================================
// HALAMAN 1: HOME PAGE (FUTUREBUILDER TERINTEGRASI API)
// =========================================================================
...
class _HomePageState extends State<HomePage> {
  ...
  void _muatUlangData() {
    setState(() {
      _futureCatatan = ApiClient.instance.getAll(); // Menggunakan ApiClient online
    });
  }
  ...
  Future<void> _konfirmasiHapus(Catatan c) async {
    final yakin = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Catatan "${c.judul}" akan dihapus dari server cloud secara permanen.'),
        actions: [
          TextButton(
    ...
    if (yakin == true && mounted) {
      try {
        await ApiClient.instance.delete(c.id!); // Hapus di server via API
        if (!mounted) return;
        _muatUlangData();
        ...
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal menghapus: ${e.message}'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Catatan Cloud Mahasiswa', style: TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Theme.of(context).colorScheme.primaryContainer,
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh, color: Colors.indigo),
            onPressed: _muatUlangData, // Tombol refresh manual untuk sistem online
          ),
          Padding(
      ...
      body: FutureBuilder<List<Catatan>>(
        ...
        builder: (context, snapshot) {
          ...
          // Penanganan Error Jaringan/HTTP yang dinamis memakai info pesan ApiException
          if (snapshot.hasError) {
            final e = snapshot.error;
            final pesanError = e is ApiException ? e.message : 'Terjadi kesalahan: $e';
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(pesanError, textAlign: TextAlign.center, style: const TextStyle(fontSize: 15)),
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    dataAsli.isEmpty ? 'Belum ada catatan di server.' : 'Tidak ada catatan kategori "$_filterTerpilih"',
...
// =========================================================================
// HALAMAN 2: REUSABLE FORM (MODE CREATE + EDIT REST API)
// =========================================================================
...
class _CatatanFormPageState extends State<CatatanFormPage> {
  ...
  Future<void> _simpan() async {
    ...
    try {
      if (_isEditMode) {
        ...
        await ApiClient.instance.update(dataTerupdate); // Put ke API
      } else {
        ...
        await ApiClient.instance.insert(dataBaru); // Post ke API
      }

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_isEditMode ? 'Catatan cloud diperbarui!' : 'Catatan cloud berhasil disimpan!'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menyimpan: ${e.message}'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditMode ? 'Edit Catatan Cloud' : 'Tambah Catatan Baru'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            ...
            FilledButton.icon(
              ...
              label: Text(
                _isEditMode ? 'Perbarui Catatan Cloud' : 'Simpan Catatan Cloud',
